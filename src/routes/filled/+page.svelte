  ...
  <Tabs style="pill" {contentClass} class="p-4">
      ...
      >
        {#each filteredEntries as [name, component]}
          <div class="flex gap-4 items-center text-lg">
            <svelte:component this={component} class="shrink-0" variation="filled" bind:size={size}/>
            {name}
          </div>
        {/each}
      </div>
    ...
    <TabItem>
      ...
      >
        {#each filteredEntries as [name, component]}
          <div class="flex gap-4 items-center text-lg">
            <svelte:component this={component} class={random_tailwind_color()} variation="filled"  bind:size={size}/>
            {name}
          </div>
        {/each}
      </div>
