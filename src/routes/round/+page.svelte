...
<h1>Svelte Google Materialdesign Icons: Round</h1>
<TableSearch
  ...
  <Tabs style="pill" {contentClass} class="p-4">
      ...
      >
        {#each filteredEntries as [name, component]}
          <div class="flex gap-4 items-center text-lg">
            <svelte:component this={component} class="shrink-0" bind:size={size} variation="round" />
            {name}
          </div>
        {/each}
      </div>
    ...
    <TabItem>
      ...
      >
        {#each filteredEntries as [name, component]}
          <div class="flex gap-4 items-center text-lg">
            <svelte:component this={component} class={random_tailwind_color()} variation="round" bind:size={size} />
            {name}
          </div>
        {/each}
      </div>
